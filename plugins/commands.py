...
async def play_loading_animation(query, reply_markup):
    """Saves from crash and plays smooth loading circle dots animation based on message type."""
    try:
        # Check if the message has photo/media or it's a plain text message
        ...
        for step in steps:
            ...
            await asyncio.sleep(0.3) # Animation Speed Control
            
    except Exception as e:
        logger.error(f"Animation function bypass: {e}")
...
# --- INTERACTIVE CALLBACKS (WITH ANIMATION SUPPORT) ---
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data == "close_data":
        ...
        # 🌟 Click par Animation load karega
        await play_loading_animation(query, premium_keyboard)
        ...
        # 🌟 Click par Animation load karega
        await play_loading_animation(query, screenshot_keyboard)
        ...
        # 🌟 Click par Animation load karega
        await play_loading_animation(query, reply_markup)
